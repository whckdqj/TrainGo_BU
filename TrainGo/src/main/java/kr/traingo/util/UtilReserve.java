...
public class UtilReserve {
	...
	public UtilReserve(int currentPage, int totalCount, int rowCount,
			int pageCount,String pageUrl,String trainnum) {
		...
		for (int i = startPage; i <= endPage; i++) {
			...
			if (i == currentPage) {
				...
				pagingHtml.append(i);
				pagingHtml.append("&trainnum="+trainnum+"'>");
				pagingHtml.append(i+"호차");
		...
		if (totalPage - startPage >= pageCount) {
				...
				pagingHtml.append("<a href="+pageUrl+"?pageNum="+ (endPage + 1) + "&trainnum="+trainnum+">");
